...
FINCLI_ARGV = ["fincli"] if _FINCLI else [PY, str(CLI_DIR / "run_cli_base.py")]
FINCLI_LABEL = "fincli" if _FINCLI else "python src_my/model_cli/run_cli_base.py"
...
NAMED_COMMANDS = {
    ...
    "latlon": {# 查询经纬度
        "argv": FINCLI_ARGV + ["latlon"],
        "arg_map": {"city": "--city"},
    },
    "weather_latlon": { #根据经纬度查询天气
        "argv": FINCLI_ARGV + ["weather_latlon"],
        "arg_map": {"lat": "--lat","lon":"--lon"},
    },
}
...
NAMED_TOOLS_SCHEMA = [
    {
        ...
        "function": {
            ...
            "description": (
                ...
                "可查 A 股年报（rag_search/list_companies）和天气（weather）和经纬度(latlon)和根据经纬度查天气(weather_latlon)。"
            ),
            "parameters": {
                "type": "object",
                "properties": {
                    "command": {
                        ...
                        "description": "rag_search（查年报，需 query+可选 stock_code/year/top_k）。"
                                       ...
                                       " weather（查天气，需 city）。"
                                       " latlon（查经纬度，需 city）。"
                                       " weather_latlon（根据经纬度查天气，需 lat + lon）。",
                    },
                    "args": {
                        "type": "object",
                        "description": "命令参数。rag_search: {query, stock_code?, year?, top_k?}; weather: {city};latlon:{city};weather_latlon:{lat,lon}",
                    },
                },
                "required": ["command"],
            },
...
BASH_TOOLS_SCHEMA = [
    {
        ...
        "function": {
            ...
            "description": (
                ...
                "查询天气 fincli weather --city 宁德。"
                "查询经纬度 fincli latlon --city 宁德。"
                "根据经纬度查询天气 fincli weather_latlon --lat 29.56 --lon 106.59。"
                "危险命令（rm/del/format/sudo/curl|sh 等）会被拦截；只允许白名单可执行文件。"
            ),
            "parameters": {
                "type": "object",
                "properties": {
                    "command": {"type": "string", "description": "完整的 shell 命令字符串(fincli list-commpanies)"},
                },
                "required": ["command"],
            },
        },
...
SYSTEM_PROMPT_NAMED = (
    ...
    "查询天气 run_cli(command='weather',args={...})"
    "查询经纬度 run_cli(command='latlon',args={...})"
    "根据经纬度查询天气 run_cli(command='weather_latlon',args={...})"
...
SYSTEM_PROMPT_BASH = (
    ...
    "查经纬度：fincli latlon --city 南京。"
    "根据经纬度查天气：fincli weather_latlon --lat 29.56 --lon 106.29。"
...
def run(client, model: str, question: str, mode: str, verbose: bool = True) -> dict:
    ...
    if msg.tool_calls:
        messages.append(msg)
        print("工具个数:",len(msg.tool_calls))
        for tc in msg.tool_calls:
            args = json.loads(tc.function.arguments or "{}")
            tool_call_log.append({"name": tc.function.name, "args": args})
            # args["command"] = tc.function.name
            if verbose:
                print(f"lines314  → [{mode}] {tc.function.name}({args})")
            try:
                argsDict = {}
                if(mode == "named"):
                    argsDict = {"command": tc.function.name, "args": args}
                elif(mode == "bash"):
                    argsDict = args
                # print("argsDict",argsDict,mode)
                result = executor(argsDict)
        ...
        *** run_cli 循环模式 ***
        ...
        try:
            if int(sel) == 2:
                ...
                mode = input("-> 请输入您希望用到的模式:bash 或者 named:\n")
