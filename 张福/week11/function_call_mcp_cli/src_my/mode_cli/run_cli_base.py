"""
run_cli.py — fincli：A股年报检索 + 天气查询 统一命令行入口
...
from src_my.weather_backend import get_weather,get_latlon,get_weather_latlon  # noqa: E402
...
def main():
    ...
    # fincli latlon ...
    p_latlon = sub.add_parser("latlon", help="查询城市经纬度")
    p_latlon.add_argument("--city", required=True, help="城市中文名，如 宁德")

    #  fincli weather_latlon ...
    p_weather_latlon = sub.add_parser("weather_latlon", help="查询城市天气，根据经纬度")
    p_weather_latlon.add_argument("--lat", required=True, help="纬度")
    p_weather_latlon.add_argument("--lon", required=True, help="精度")
    ...
    if args.cmd == "search":
        ...
        print(get_latlon(args.city))
    elif args.cmd == "weather_latlon":
        print(get_weather_latlon(args.lat,args.lon))
