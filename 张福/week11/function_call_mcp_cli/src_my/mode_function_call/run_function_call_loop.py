"""
run_function_call_loop.py — 方式一：Function Call（模型原生函数调用）
循环模式
"""
...
from src_my.weather_backend import call_weather ,call_latlon,call_weather_latlon # noqa: E402
...
TOOLS_SCHEMA = [
    ...
    },
    {
        "type": "function",
        "function": {
            "name": "get_weather_latlon",
            "description": "查询指定经纬度的当前天气及未来3天预报。经纬度用字符串浮点型数字，如 '29.56'、'106.52'。",
            "parameters": {
                "type": "object",
                "properties": {
                    "lat": {"type": "string", "description": "纬度，如 '29.56'"},
                    "lon": {"type": "string", "description": "精度，如 '106.56'"},
                },
                "required": ["lat","lon"],
            },
        },
...
# python run_function_call.py  -q 通过重庆的经纬度查询天气

TOOL_DISPATCH = {
    "search_annual_report": search_annual_report,
    "list_companies": list_companies,
    "get_weather": call_weather, # 经纬度 + 天气
    "get_latlon":   call_latlon, # 单独获取经纬度
    "get_weather_latlon":   call_weather_latlon #通过经纬度查询天气
}
...
SYSTEM_PROMPT = (
    ...
    "涉及天气时调用 get_weather或者get_weather_latlon。本回合你可以一次调用多个工具。"
    "涉及经纬度查询调用 get_latlon。本回合你可以一次调用多个工具。"
)
def json_to_argparser(json_config: dict):
    import argparse
    parser = argparse.ArgumentParser()
    for key, val in json_config.items():
        # 根据值类型自动设置参数规则
        parser.add_argument(f"--{key}", help="", type=type(val), default=val)
    ...
    if msg.tool_calls:
        ...
        for tc in msg.tool_calls:
            ...
            if fn is None:
                ...
                try:
                    # 工具执行！！
                    result = fn(json_to_argparser(args))
        ...
        *** run_cli 循环模式 ***
        ...
        try:
            if int(sel) == 2:
                ...
                import argparse
                parser = argparse.ArgumentParser(description="方式一：Function Call")
                parser.add_argument("--question", "-q", help="单个问题",default=question)
                parser.add_argument("--demo", action="store_true", help="跑内置示例问题集")
                parser.add_argument("--provider", default="dashscope", choices=PROVIDERS.keys())
                parser.add_argument("--quiet", action="store_true", help="少输出（被 compare.py 调用时用）")
                parser.add_argument("--json", action="store_true", help="输出 JSON（供 compare.py 解析）")
